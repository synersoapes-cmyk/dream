...
- 符石 / 星石 / 最优解逻辑在用户提供正式数据前，不继续做猜测型穷举
- 真实装备素材本体先保持空占位，不主动补虚拟素材；等用户提供 CC 素材包后再批量导入
- 符石系统、星石系统、星相互合、最优符石组合默认解先保持数据占位；等用户提供正式规则数据后再接入，不继续用当前临时口径扩展
...
- 已完成补充：
  ...
  - 装备图链路已从外部随机图切到本地 `equipment-art` resolver，当前支持“按装备名命中素材映射，不命中则按部位回退”
  - 已补统一 artwork helper，前台当前装备卡、待确认卡、新品卡、实验席位卡与相关详情弹窗都已统一优先走本地装备图展示，不再被历史 OCR 截图或旧外链主图覆盖
  - 已拆出独立 `simulator-equipment-artwork-manifest`，并预留 `public/simulator/equipment-art/` 目录，后续可直接按 manifest 批量挂真实素材
...
- 未完成：
  ...
  - 梦幻真实装备素材文件本体先空着，当前仅完成 resolver、名称归一化、导入脚本与展示层接入；等用户提供 CC 素材包后再批量导入实际素材文件与别名映射
...
- 已完成：
  ...
  - 右侧最终面板已展示关键结果字段。
  - 右侧最终面板已进一步重排为“角色速览 → 核心三值 → 输出/生存/辅助分组 → 关键字段来源”的阅读顺序，更贴近梦幻玩家平时看面板的习惯，同时保留原有来源拆解能力。
  - 右侧最终面板已新增“试算语境”摘要，当前会直接展示技能、我方阵法、我方五行、当前目标、目标阵法、法伤结果、目标法防结果，减少“数值看到了但不知道是按什么条件算出来的”理解成本。
- 剩余迁移：
  - 右侧字段编排继续向真实梦幻角色截图靠齐，统一迁入 `Unified-Next-01`。
...
- 真实梦幻装备素材正式入仓
  - 当前素材文件本体先空着，只保留导入链路和目录占位
  - 等用户提供 CC 素材包后，再批量接入真实装备图片
  - 等用户提供素材别名/映射后，再利用 import + alias-file 链路补齐 artwork 静态映射表
...
- 不再把实验室重新扩成多对比席位横向模型
- 普通换装弹窗与实验室槽位选择器继续只消费 `active` 正式库存
- 不主动填充真实装备素材；素材包未提供前，`public/simulator/equipment-art/` 继续保持目录占位
