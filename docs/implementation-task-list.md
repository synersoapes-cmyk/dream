...
- 已完成：
  ...
  - 待确认详情页的人工复核与改动比对
- 部分完成：
  - OCR 已有统一装备识别链路，但暂未看到针对“藏宝阁截图 / 聊天框预览图”显式区分模式或专门 prompt 分支
- 未完成：
  - 暂未形成面向“截图类型”的专门识别优化策略说明和前台反馈
...
- 当前可记为 `6 / 6` 已完成：
  - `M9-01` 已补 `通天河` fallback 目标模板，并会在副本目标面板自动补齐缺失预置
...
- `通天河` 通过前端 fallback 模板与模板合并逻辑进入副本目标面板，不依赖后台模板预置是否已同步。
...
- `2026-04-12`：已完成当前轮口径统一，前台、store 与试算均以“样本席位 + 2 个对比席位”为正式能力边界。
