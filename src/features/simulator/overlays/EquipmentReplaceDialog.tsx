...
export function EquipmentReplaceDialog() {
  ...
  const [saveReplacementError, setSaveReplacementError] = useState<string | null>(null);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setSaveReplacementError(error instanceof Error ? error.message : '替换失败');
    } finally {
      ...
      {previewMode && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 z-[10001] max-h-[85vh] w-[700px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto"
          >
            <div className="overflow-hidden rounded-2xl border-2 border-yellow-700/60 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 shadow-2xl">
              <div className="flex items-center justify-between border-b border-yellow-700/60 bg-gradient-to-r from-yellow-900/50 to-yellow-800/30 px-6 py-4">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-yellow-400/70">Equipment Replacement Preview</p>
                </div>
                <button
                  onClick={handleCancel}
              ...
              <div className="space-y-5 p-6">
                <div className="grid grid-cols-[1fr_auto_1fr] gap-4">
                  <div className="rounded-xl border border-yellow-800/30 bg-slate-900/60 p-4">
                    <div className="mb-3 text-center text-xs text-yellow-400/60">当前装备</div>
                    {current ? (
                      <>
                        <div className="mb-2 text-center font-bold text-yellow-100">{current.name}</div>
                      ...
                      <div className="py-8 text-center text-sm italic text-yellow-600/40">无装备</div>
                    )}
                  </div>
                  ...
                  <div className="rounded-xl border-2 border-yellow-600/50 bg-gradient-to-br from-yellow-900/40 to-yellow-800/20 p-4 shadow-lg">
                    <div className="mb-3 text-center text-xs font-semibold text-yellow-400">新装备</div>
                    <div className="mb-2 text-center font-bold text-yellow-100">{newEquip.name}</div>
                    ...
                    {Object.entries(statsDiff.attributes).map(([key, diff]) => {
                      ...
                      return (
                        <div key={key} className="flex items-center justify-between py-1 text-sm">
                          ...
                          <span className={`flex items-center gap-1 font-semibold ${diff > 0 ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="rounded-xl border border-yellow-700/50 bg-gradient-to-br from-yellow-900/20 to-yellow-800/10 p-4">
                  ...
                  <div className="mb-3 grid grid-cols-3 gap-4">
                    <div className="rounded-lg border border-yellow-800/30 bg-slate-900/50 p-3 text-center">
                      <div className="mb-1.5 text-xs text-yellow-400/70">物理伤害</div>
                    ...
                    <div className="rounded-lg border border-yellow-800/30 bg-slate-900/50 p-3 text-center">
                      <div className="mb-1.5 text-xs text-yellow-400/70">法术伤害</div>
              ...
              <div className="border-t border-yellow-800/40 bg-slate-900/60 px-6 py-4">
                {saveReplacementError && (
                  <div className="mb-3 text-center text-xs text-red-300">{saveReplacementError}</div>
                )}
